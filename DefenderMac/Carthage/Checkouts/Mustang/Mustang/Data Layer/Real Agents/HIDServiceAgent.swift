    ...
    internal func confirmChangeWithVendorId(_ vendorId: Int, productId: Int, locationId: UInt32, onSuccess: @escaping (_ data: [[UInt8]]?) -> (), onFail: () -> ()) {
        ...
        whenDeviceIsOpen(vendorId, productId: productId, locationId: locationId) { (device) in
            ...
            self.sendToDevice(device, data:[0x1c, 0x03], terminator: [0x00, 0x00, 0x1c, 0x00]) { (response) in
                self.DebugPrint(" Confirmed change\n")
                onSuccess(response)
            }
        }
    }
...
//        classesToMatch[kIOHIDProductIDKey] = productId ?? 0xff
    ...
    fileprivate func printDevice(_ devicePtr: HIDDeviceType) {
        ...
        DebugPrint(" vendor: 0x\(vendorId != nil ? String(format: "%04x", vendorId!) : "Unknown")")
        DebugPrint(" product Id: 0x\(productId != nil ? String(format: "%04x", productId!) : "Unknown")")
        DebugPrint(" version 0x\(version != nil ? String(format: "%04x", version!) : "Unknown")")
        DebugPrint(" unique Id: 0x\(uniqueId != nil ? String(format: "%04x", uniqueId!) : "Unknown")")
        DebugPrint(" manufacturer: \(manufacturer != nil ? manufacturer! : "Unknown")")
        DebugPrint(" product: \(product != nil ? product! : "Unknown")")
        DebugPrint(" serial: \(serial != nil ? serial! : "Unknown")")
        DebugPrint(" location: 0x\(location != nil ? String(format: "%04x", location!) : "Unknown")")
        DebugPrint(" transport: \(transport != nil ? transport! : "Unknown")")
        DebugPrint(" in report: \(maxInReport != nil ? String(format: "%d", maxInReport!) : "Unknown")")
        DebugPrint(" out report: \(maxOutReport != nil ? String(format: "%d", maxOutReport!) : "Unknown")")
        DebugPrint(" feature report: \(maxFeatureReport != nil ? String(format: "%d", maxFeatureReport!) : "Unknown")")
        DebugPrint(" report interval: \(reportInterval != nil ? String(format: "%d", reportInterval!) : "Unknown")")
    }
