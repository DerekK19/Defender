...
internal class USBServiceAgent: BaseServiceAgent, USBServiceAgentProtocol {
    ...
    fileprivate func handleDeviceAdded(_ iterator: io_iterator_t) {
        ...
        if usbService != 0 {
            DebugPrint("USB - Something was added : \(usbService)")
    ...
    fileprivate func handleGeneralInterest(_ messageType: natural_t, service: io_service_t, messageArgument: UnsafeMutableRawPointer?) {
        ...
        case kIOMessageServiceIsTerminated:
            DebugPrint("USB - Something was removed : \(service) \(messageArgument)")
            if service != 0 {
                DebugPrint("USB Device removed: \(service).")
                startDetectingDevicesWithVendorId(vendorId)
            }
        case kIOMessageServiceIsAttemptingOpen:
            DebugPrint("USB - Something will open - service:\(service) device:\(lastDeviceService) interface:\(lastInterfaceService)) \(messageArgument)")
        case kIOMessageServiceWasClosed:
            DebugPrint("USB - Something did close - service:\(service) device:\(lastDeviceService) interface:\(lastInterfaceService)) \(messageArgument)")
            var tryToOpenInterface = false
            if lastDeviceService == service {
                if let lastDevice = lastDevice {
                    DebugPrint("Closing the device")
            ...
            if lastInterfaceService == service || tryToOpenInterface{
                if let lastInterface = lastInterface, let lastCompletion = lastCompletion {
                    DebugPrint("Attempting Open again \(lastInterfaceService) (\(messageArgument))")
                    if let success = self.openInterface(lastInterface) {
                        lastCompletion(success)
                    } else {
                        DebugPrint("Still not able to open the interface")
        ...
        default:
            debugLog("USB General interest: \(service) 0x\(String(format: "%08x", messageType)) (\(descriptionForMessageType(messageType)))")
        }
    }
    ...
    fileprivate func executeOnDevice(_ vendorId: Int,
        ...
        repeat {
            ...
            if (usbService != 0) {
                ...
                debugLog("deviceName: \(deviceName ?? "Unknown")")
                ...
                if let devicePtr = getDeviceForService(usbService, interface: interface) {
                    ...
                    if let thisProductId = thisProductId, let thisLocationId = thisLocationId {
                        ...
                        repeat {
                            ...
                            if usbInterface != 0 {
                                DebugPrint(" Interface: 0x\(String(format: "%08x", usbInterface))")
...
//                                                
//                                                DebugPrint(" Endpoints (\(endpointCount))")
//                                                if endpointCount > 0 {
//                                                    for pipeRef: UInt8 in 1 ... endpointCount {
//                                                        self.debugLog("pipeRef: \(pipeRef)")
//
...
//                                        }
    ...
    fileprivate func getProductIdforDevice(_ devicePtr: USBDeviceType) -> UInt16? {
        ...
        if kr != KERN_SUCCESS {
            ...
            DebugPrint("Product Id: 0x\(String(format: "%08x", productId))\n")
        }
        return productId
    }
    ...
    fileprivate func getLocationForDevice(_ devicePtr: USBDeviceType) -> UInt32? {
        ...
        if kr != KERN_SUCCESS {
            ...
            debugLog("Location ID: 0x\(String(format: "%08x", locationId))\n")
        }
        return locationId
    }
    ...
    fileprivate func openInterface(_ interfacePtr: USBInterfaceType) -> Bool? {
        ...
        if kr == kIOReturnSuccess
        {
            DebugPrint("Opened interface")
            return true
        } else if kr == kIOReturnExclusiveAccess {
            DebugPrint("  USBInterfaceOpen failed. Exclusive access")
    ...
    fileprivate func DebugPrint(_ name: String?,
                            ...
                            audio: AnyObject?) {
        DebugPrint(" ")
        DebugPrint("Device name is \(name ?? "Unknown")")
    ...
    fileprivate func DebugPrint(_ devicePtr: USBDeviceType) {
        printDevice(devicePtr)
    }
    fileprivate func DebugPrint(_ interfacePtr: USBInterfaceType) {
        printInterface(interfacePtr)
    }
    fileprivate func DebugPrint(_ audioPtr: AnyObject) {
        //printAudio(audioPtr)
    }
    
    func printDevice(_ devicePtr: USBDeviceType) {
        let device = devicePtr.pointee.pointee
        var kr: kern_return_t = 0
...
//        logError("GetBusFrameNumber returned", kr: kr)
        ...
        DebugPrint("Vendor is 0x\(String(format: "%04x", deviceVendor))")
        DebugPrint("Product is 0x\(String(format: "%04x", deviceProduct)) - \(getProducNameforDevice(devicePtr) ?? "Unknown")")
        DebugPrint("Release number is 0x\(String(format: "%04x", releaseNumber))")
        DebugPrint("Class is \(deviceClass)")
        DebugPrint("SubClass is \(deviceSubClass)")
        DebugPrint("Protocol is \(deviceProtocol)")
        DebugPrint("Location is 0x\(String(format: "%08x", thisLocationId))")
        DebugPrint("Address is \(String(format: "%04x", address))")
        DebugPrint("Configs is \(configCount)")
        DebugPrint("Config is \(configNum)")
        if config != nil {
            DebugPrint(" Config descriptor is \(config?.pointee.bDescriptorType)")
            DebugPrint(" Config interfaces is \(config?.pointee.bNumInterfaces)")
            DebugPrint(" Config value is \(config?.pointee.bConfigurationValue)")
            DebugPrint(" Configuration is \(config?.pointee.iConfiguration)")
            DebugPrint(" Config attributes is \(config?.pointee.bmAttributes)")
            DebugPrint(" Config power is \(config?.pointee.MaxPower)")
        }
        DebugPrint("Speed is \(speed)")
        DebugPrint("Power is \(busPower)")
    }
    
    func printInterface(_ intInterfacePtr: USBInterfaceType) {
        ...
        DebugPrint("  Vendor is 0x\(String(format: "%04x", interfaceDeviceVendor))")
        DebugPrint("  Product is 0x\(String(format: "%04x", interfaceDeviceProduct))")
        DebugPrint("  Release number is 0x\(String(format: "%04x", interfaceReleaseNumber))")
        DebugPrint("  Interface number is \(interfaceNumber)")
        DebugPrint("  Class is \(interfaceClass) (\(descriptionForInterfaceClass(interfaceClass)))")
        DebugPrint("  SubClass is \(interfaceSubClass) (\(descriptionForInterfaceClass(interfaceClass, subClass: interfaceSubClass)))")
        DebugPrint("  Protocol is \(interfaceProtocol)")
        DebugPrint("  Config value is \(configValue)")
        DebugPrint("  Alternate is \(alternateSetting)")
        DebugPrint("  Async port is 0x\(String(format: "%08x", aPort))")
        DebugPrint("  Async event source is \(eSource)")
        DebugPrint("  Endpoints is \(endpointCount)")
        
        if endpointCount > 0 {
            for endpointNumber: UInt8 in 1...endpointCount {
                DebugPrint("   Endpoint: \(endpointNumber)")
...
//                    if kr == kIOReturnSuccess {
//                        DebugPrint("    Direction: \(direction) (\(descriptionForDirection(direction)))")
//                        DebugPrint("     Transfer type is \(transferType) (\(descriptionForTransferType(transferType)))")
//                        DebugPrint("     Packet size is \(packetSize)")
