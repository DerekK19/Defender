...
internal class MustangAgent {
    ...
    // MARK: Serializationb functions
    internal class func serialize(_ data: [[UInt8]], intoPresets presets: inout [DLPreset]) {
        for setting in data {
                ...
                case 0x04:
                    ...
                    if presets.filter({ $0.number == number }).count == 0 {
                        ...
                        if let name = String(bytes: Array(setting[16...48]), encoding: String.Encoding.utf8)?.trimmingCharacters(in: CharacterSet.init(charactersIn: "\0")) {
                            presets.append(DLPreset(withNumber: number, name: name, current: current))
                        }
                    }
                case 0x05:
                    // AMP Settings
